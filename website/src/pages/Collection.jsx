...
import { subscribeCardShowcase, subscribeCardsPage } from "../lib/data.js";
...
export default function Collection() {
  ...
  useEffect(() => subscribeCardShowcase(6, (value) => setShowcase((current) => ({ ...current, ...value }))), []);
  ...
  return (
    <Page>
      <Header title="Card Gallery" subtitle="Browse Aron cards with collectible-grade presentation, rarity badges, and wishlist demand." search={search} setSearch={setSearch} />
      <FeatureStrip title="Featured Cards" cards={showcase.featured.slice(0, 3)} />
      <div className="mb-8 grid gap-6 lg:grid-cols-3">
        <MiniList title="Top Wishlisted" cards={showcase.topWishlisted.slice(0, 4)} />
        <MiniList title="Rare Finds" cards={showcase.rareFinds.slice(0, 4)} />
        <MiniList title="Recently Added" cards={showcase.recentlyAdded.slice(0, 4)} />
      </div>
...
function FeatureStrip({ title, cards }) {
  ...
  return (
    <div className="glass rounded-2xl p-5">
      ...
      <div className="mt-4 grid gap-3">
        ...
        )) : <div className="rounded-xl border border-dashed border-white/15 p-4 text-sm text-white/54">Waiting for public data.</div>}
      </div>
    </div>
  );
}
