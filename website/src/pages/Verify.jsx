...
function VerificationMatches({ matches }) {
  ...
  return (
    <div className="mt-3 rounded-3xl border border-amber-300/20 bg-amber-300/10 p-4 text-left text-sm text-amber-100">
      <div className="font-black text-white">Matching verification data</div>
      <div className="mt-2 space-y-2">
        {matches.map((match, index) => (
          <div key={`${match.type}-${match.matchedUserId}-${index}`} className="rounded-2xl border border-white/10 bg-black/15 p-3">
...
function VerifyMascot() {
  return (
    <div className="relative hidden min-h-[560px] lg:block">
      ...
      />
      <div className="absolute left-1/2 top-[22%] -translate-x-[115%] -rotate-6 rounded-2xl border border-cyan/20 bg-cyan/10 px-4 py-3 shadow-card backdrop-blur">
  ...
  return (
    <div className="glass mx-auto max-w-3xl rounded-[2rem] p-8 text-center">
      ...
      {result?.reasons?.length ? (
        <div className="mt-5 rounded-3xl border border-white/10 bg-white/5 p-4 text-left text-sm text-white/62">
          {result.reasons.join(", ")}
        </div>
      ) : null}
  ...
  return (
    <Page className="py-12 lg:py-16">
      <div className="mx-auto grid max-w-6xl items-center gap-8 lg:grid-cols-[minmax(0,560px)_1fr]">
        <section className="verify-rules-panel rounded-[2rem] p-6 shadow-card sm:p-8">
          ...
          {error ? <div className="mt-6 rounded-2xl border border-rose/30 bg-rose/10 px-4 py-3 text-sm font-semibold text-rose">{error}</div> : null}
          {!hasVerificationApi() ? <div className="mt-6 rounded-2xl border border-amber-300/30 bg-amber-300/10 px-4 py-3 text-sm font-semibold text-amber-200">Verification server URL is missing.</div> : null}
          ...
          {token ? (
            ...
            <div className="mt-7 rounded-2xl border border-violet/30 bg-violet/15 px-4 py-3 text-center text-sm font-bold text-violet">
