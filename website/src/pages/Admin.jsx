...
function AdminInner() {
  ...
  async function run(label, action) {
    ...
    try {
      ...
      if (access.userId) setVerification(await getVerificationProfile(access.userId));
    } catch (error) {
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Panel title="Access" icon={ShieldCheck}>
          <div className="grid gap-3">
            ...
            <Button variant="ghost" icon={Search} onClick={() => run("Verification lookup", async () => { setVerification(await getVerificationProfile(access.userId)); return "Verification profile loaded."; })}>Load Verification</Button>
            {verification ? (
              <div className="rounded-lg border border-white/10 bg-white/5 p-3 text-sm text-white/64">
                <div className="font-bold text-white">{verification.user?.username || access.userId}</div>
                <div>Status: {verification.status?.status || "unverified"} | Risk: {verification.status?.riskScore ?? verification.status?.suspicionScore ?? "n/a"}</div>
                <div>Reasons: {verification.status?.riskReasons?.join(", ") || verification.status?.reason || "none"}</div>
                <div className="mt-2 text-xs text-white/44">Attempts: {verification.attempts.slice(0, 5).map((attempt) => `${attempt.status}:${attempt.riskScore}`).join(" | ") || "none"}</div>
              </div>
            ) : null}
