...
const ACCOUNT_STATUSES = ["unverified", "verified", "flagged", "quarantined", "blocked"];
...
export async function getVerificationProfile(userId) {
  const id = cleanId(userId);
  if (!id) return null;
  const [user, status, attempts] = await Promise.all([
    readValue(`verificationUsers/${id}`, null),
    readValue(`meta/accountSecurity/userStatuses/${id}`, null),
    readValue(`verificationAttemptsByUser/${id}`, {})
  ]);
  return {
    user,
    status,
    attempts: Object.entries(normalizeCollection(attempts))
