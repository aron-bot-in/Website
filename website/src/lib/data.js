...
export const dataStatuses = {
  loading: { source: "loading", label: "Loading" },
  live: { source: "live", label: "Live data" },
  fallback: { source: "fallback", label: "Cached data" },
  limited: { source: "limited", label: "Limited data" },
  unavailable: { source: "unavailable", label: "Unavailable" },
  error: { source: "error", label: "Data offline" }
};
...
function sanitizePublicUser(user, id) {
  ...
  return {
    ...
    profile: {
      ...
      badges: Array.isArray(profile.badges) ? profile.badges.slice(0, 5).map(String) : []
    },
    stats: {
