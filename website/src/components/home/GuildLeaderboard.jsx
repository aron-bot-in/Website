      ...
      <div className="rounded-lg border border-white/10 bg-panel/82 p-5 shadow-card sm:p-6">
        ...
        <div className="mt-6 grid gap-3">
          {visibleGuilds.map((guild) => (
            <article key={guild.id || guild.rank} className="grid gap-4 rounded-lg border border-white/10 bg-white/[0.035] p-4 transition hover:border-cyan/26 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center">
              ...
              <div className="w-fit rounded-lg border border-cyan/20 bg-cyan/[0.08] px-3 py-2 text-lg font-black text-cyan sm:justify-self-end">{compactNumber(guild.score)}</div>
            </article>
          ))}
          {!visibleGuilds.length ? (
            <div className="rounded-lg border border-dashed border-white/15 bg-white/[0.03] p-6 text-white/58">
              Guild leaderboard data is not available yet.
            </div>
          ) : null}
        </div>
      </div>
